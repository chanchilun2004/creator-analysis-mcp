...
- **Default output (fastest): a short chat summary (3–5 sentences) + a full-report link.** After analysis, call `creator_report_link({username, analysis})` (analysis JSON shape in references/api-reference.md) and present the returned URL prominently as 「📊 開啟完整報告」. The server renders the complete visual report (avatar, reel thumbnails, growth chart) instantly — do NOT also render an inline dashboard unless the user asks for one.
- Only when the user explicitly asks for an inline dashboard: single-creator mode copies the fixed template in references/widget-template.md verbatim replacing ONLY `const D` (no inline avatar needed — the full report has it); multi-creator comparisons and the plain-text fallback follow references/report-template.md: header card with creator identity + verdict badge, metric stat cards, score bars per dimension, past-collaboration list, risk flags, and for multi-creator mode a side-by-side comparison table with a ranked recommendation.
